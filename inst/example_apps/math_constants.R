...
pi_digits <- system.file("example_apps", "pi_digits.txt", package = "ggcircos")
phi_digits <- system.file("example_apps", "phi_digits.txt", package = "ggcircos")
x <- dget(pi_digits)
...
plot <- ggvis() %>%
  ...
  hide_axis("x") %>%
  hide_axis("y") %>%
  ggvis::set_options(width = 800, height = 800, keep_aspect = TRUE)
#  bind_shiny("plot")
