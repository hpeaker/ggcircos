...
add_ribbons.reactive <- function(vis, seq_df, name_from, name_to, pos_from_start, pos_from_end,
  ...
  ribbons_df <- reactive({
    ...
    pos_from_start_sub <- eval(substitute(pos_from_start, parent.env(parent.env(environment()))), data, parent.frame())
    pos_from_end_sub <- eval(substitute(pos_from_end, parent.env(parent.env(environment()))), data, parent.frame())
    pos_to_start_sub <- eval(substitute(pos_to_start, parent.env(parent.env(environment()))), data, parent.frame())
    pos_to_end_sub <- eval(substitute(pos_to_end, parent.env(parent.env(environment()))), data, parent.frame())
