...
donors <- read.csv("../icgc_example/clean_data/donors_clean.csv") %>% filter(icgc_donor_id %in% use_donors)
snp <- read.csv("../icgc_example/clean_data/snp_clean.csv") %>% filter(icgc_donor_id %in% use_donors, chromosome != "MT")
struct <- read.csv("../icgc_example/clean_data/struct_clean.csv") %>% filter(icgc_donor_id %in% use_donors)
cnv <- read.csv("../icgc_example/clean_data/cnv_clean.csv") %>% filter(icgc_donor_id %in% use_donors)
...
#       
#       paste(
#         paste(transcript_data$transcript_affected, transcript_data$consequence_type, sep = ": "),
#         collapse = "<br>"
#       )
#       
#     }
...
#   }
  ...
  ggvis() %>%
    ...
    layer_points(data = snp_plot_data, ~sin(theta) * r, ~cos(theta) * r, fill = ~mutation_type,
                 key := ~id, size := 10, size.hover := 30, strokeOpacity := 0, strokeOpacity.hover := 1) %>%
...
#               strokeWidth := 1, strokeWidth.hover := 2) %>%
    layer_paths(data = struct_plot_data, ~sin(theta) * r, ~cos(theta) * r, stroke = ~inter,
                strokeWidth := 1, strokeWidth.hover := 2, interpolate := "basis") %>%
